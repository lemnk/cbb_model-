...
class RandomForestModel:
    """
    Random Forest model for both classification and regression tasks.
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if X_val is not None and y_val is not None:
            ...
            if self.task == 'classification':
                ...
                print(f"✅ Random Forest ({self.task}) fitted. Validation accuracy: {val_score:.4f}")
            else:
                val_score = self.model.score(X_val_scaled, y_val)
                print(f"✅ Random Forest ({self.task}) fitted. Validation R²: {val_score:.4f}")
        else:
            print(f"✅ Random Forest ({self.task}) fitted.")
